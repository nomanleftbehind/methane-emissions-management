...
pub fn establish_connection(db_kind: DatabaseKind) -> DbPool {
    ...
    let postgres_db_port = env::var("POSTGRES_DB_PORT").expect("POSTGRES_DB_PORT must be set");
    ...
    let database_url = format!(
        "postgres://{}:{}@{}:{}/{}",
        postgres_user, postgres_password, postgres_db_host, postgres_db_port, postgres_db
    );
