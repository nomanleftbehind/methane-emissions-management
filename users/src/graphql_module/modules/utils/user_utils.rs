...
impl Guard for RoleGuard {
    async fn check(&self, context: &Context<'_>) -> Result<(), async_graphql::Error> {
        if context.data_opt::<AuthRole>() == Some(&AuthRole::Admin)
            || context.data_opt::<AuthRole>() == Some(&self.role)
        {
            Ok(())
        } else {
...
lazy_static! {
    static ref PASSWORD_SECRET_KEY: String =
        var("PASSWORD_SECRET_KEY").expect("You must set PASSWORD_SECRET_KEY");
}
