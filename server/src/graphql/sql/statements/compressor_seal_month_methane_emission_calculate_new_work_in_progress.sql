...
-- 					(
						SELECT
							...
							csmme.month,
							CASE
								WHEN csmme.control_device IS NULL THEN
								-- Uncontrolled
									CASE
										...
										ELSE 'ROUTINE'::methane_emission_category
									END
								ELSE
								-- Controlled
									CASE
										WHEN cpr.type = 'RECIPROCATING' AND csmme.identity = 'CRANKCASE' THEN
											CASE
												...
												-- When a compressor seal vent control is not operating, compressor seal vents are considered nonroutine vents.
												ELSE 'NONROUTINE'::methane_emission_category
											END
										ELSE
											CASE
												WHEN csmme.reason IN ('PLANNED_MAINTENANCE', 'UNPLANNED_MAINTENANCE') THEN 'NONROUTINE'::methane_emission_category
												ELSE 'FUGITIVE'::methane_emission_category
											END
									END
							END as category,
						...
						SELECT
							...
							csmme.control_device,
							csmme.reason,
							UNNEST(ARRAY['NONCRANKCASE', 'CRANKCASE', 'EMISSION_SURVEY']) as identity,
							UNNEST(ARRAY[csmme.gas_volume, csmme.gas_volume_crankcase, csmme.emission_survey_gas_volume]) as gas_volume,
							UNNEST(ARRAY[csmme.c1_volume, csmme.c1_volume_crankcase, csmme.emission_survey_c1_volume]) as c1_volume,
							UNNEST(ARRAY[csmme.co2_volume, csmme.co2_volume_crankcase, csmme.emission_survey_co2_volume]) as co2_volume
																					...
																					(
																						...
																						FROM
																							...
																							LEFT OUTER JOIN (
																								...
																								FROM compressor_seal_test_cleaned cst
																								WHERE cst.testing_point <> 'CRANKCASE'
																							
																							) cst ON cst.compressor_seal_id = cs.id AND cs.date BETWEEN cst.start_date AND cst.end_date
																							GROUP BY
																							cs.id,
																							cs.site_id,
																							cs.date,
																							...
																							) cst_ckc ON cst_ckc.compressor_seal_id = csmme.id AND csmme.date BETWEEN cst_ckc.start_date AND cst_ckc.end_date
																							) csmme
																							) csmme
																							) csmme
					...
					INNER JOIN compressor cpr ON cpr.id = csmme.id AND NOT (/*controlled emission from non-crankcase testing point is destroyed or recovered so it's filtered out here*/csmme.control_device IS NOT NULL AND csmme.reason IS NULL AND csmme.identity = 'NONCRANKCASE')
-- 					) csmme
-- 			) csmme
-- 	) csmme
-- GROUP BY
